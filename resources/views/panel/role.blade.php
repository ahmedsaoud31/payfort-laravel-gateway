...
<div class="col-sm-12">
	<div class="row">
		...
		<div class="col-lg-12">
			<form role="form" method="POST" action="{{ url ('/panel/auth/role') }}" enctype="multipart/form-data">
				{{ csrf_field() }}
				<div class="row">
					<div class="col-lg-12">
						<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
							<label>Add new role</label>
							<input class="form-control" name="name" value="{{ old('name') }}" placeholder="Enter a role">
							@if ($errors->has('name'))
	...
	<div class="row">
		<div class="col-sm-12">
			<table class="table table-condensed">
				...
				<tbody>
				@foreach($role = \App\Tables\Role::paginate(20) as $key=>$row)
					<tr data-id="{{$row->id}}">
						<td>{{ $row->id }}</td>
...
$(function(){
	$('.delete').click(function(){
		...
		swal({
		  ...
		  confirmButtonText: 'Yes, delete it!'
			...
			$.ajax({
				url: "{{url('/ajax/panel/auth/role/delete')}}",
				type: "POST",
				data: sendData,
	...
	$(".name, .desc").focusout(function () {
		...
		if(thisElem.find('.name').attr('data-name') != sendData.name
			...
			$.ajax({
				url: "{{url('/ajax/panel/auth/role/edit')}}",
				type: "POST",
				data: sendData,
				beforeSend: function ( xhr ) {
					googleBar.start();
